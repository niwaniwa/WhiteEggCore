...
public class LoadLanguage implements Base {
    ...
    private void copyLangFiles(boolean send) {
        for (LanguageType type : LanguageType.values()) {
            ...
            if (path.exists()) {
                if (send) {
                    WhiteEggCore.logger.info(" " + type.getString() + " : file already exists ");
                }
                continue;
            }
            if (send) {
                WhiteEggCore.logger.info(" " + type.getString() + " : loading now...");
            }
            ...
            if (send) {
                WhiteEggCore.logger.info(" " + type.getString() + " : " + (path.exists() ? "complete" : "failed"));
            }
        }
    }
